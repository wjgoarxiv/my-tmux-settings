...
Supports **macOS / WSL2 / Linux** (hardcoded config), **Windows native tmux** (MSYS2 + Git Bash), and **Windows psmux** (PowerShell).
...
|---|---|---|---|
| **macOS / WSL2 / Linux** | Hardcoded config | Any | Tokyonight Night |
| **Windows (MSYS2 native tmux)** | Hardcoded config via Node.js | MSYS2 zsh + Git Bash configs | Tokyonight Night |
| **Windows (psmux)** | Hardcoded config via Node.js | PowerShell | Tokyonight Night |
...
1. Copies `tmux.conf` → `~/.tmux.conf`
2. Installs `sysinfo.sh` CPU/MEM widget
...
4. Generates hardcoded Tokyonight Night `~/.tmux.conf` via Node.js
...
### `install.ps1` (psmux)
1. Generates hardcoded Tokyonight Night `~/.tmux.conf` via Node.js
...
Edit `customize/gen-config.js` — change the Tokyonight Night hex values at the top, then:
```bash
...
```
I want to install the tmux Tokyonight Night config from https://github.com/wjgoarxiv/my-tmux-settings.
...
STEP 3 — Install:        bash ~/my-tmux-settings/install.sh
  → Auto-installs: ~/.tmux.conf, sysinfo.sh
...
STEP 2 — Run installer (from Git Bash):
  ...
  c) Generates Tokyonight Night ~/.tmux.conf via gen-config.js --windows
...
CRITICAL WINDOWS PITFALLS (the installer handles these, but know them):
  ...
  5. MSYS2 doesn't have tmux-256color terminfo — use screen-256color.
  6. Set USERPROFILE/HOMEDRIVE/HOMEPATH in tmux.conf for Windows-native programs (conda, Python).
  7. MSYS2 tmux does NOT fully inherit the Windows user PATH. The installer adds
     MSYS2_PATH_TYPE=inherit to the tmux alias and explicit PATH entries to .zshrc.
  8. Windows console programs (node, claude) need winpty for TTY support inside
...
STEP 3 — Install:        powershell -File $HOME/my-tmux-settings/install.ps1
  → Generates hardcoded Tokyonight Night ~/.tmux.conf, installs sysinfo.sh
STEP 4 — Font:           choco install -y nerd-fonts-JetBrainsMono
STEP 5 — Set Windows Terminal font to "JetBrainsMono Nerd Font"
STEP 6 — Run: psmux

Confirm: is the Tokyonight Night status bar visible at the top
